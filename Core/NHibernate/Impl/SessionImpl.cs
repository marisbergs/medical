...
namespace NHibernate.Impl
{
	...
	public sealed class SessionImpl : ISessionImplementor, ISerializable, IDeserializationCallback
	{
			...
			for( int i = 0; i < values.Length; i++ )
			{
				if( !nullability[ i ] && checkability[ i ] && values[ i ] == null )
				{
					throw new PropertyValueException(
						"AAAAAAAAAAAAAAAAAA not-null property references a null or transient value: ",
        ...
        {
            return this.Load<T>(id, lockMode);
        }
        ...
        public bool Contains(System.Type theType, object id)
        {
            CheckIsOpen();
            ...
            IEntityPersister persister = GetClassPersister(theType);
        	EntityKey key = new EntityKey( id, persister );
		...
		{
			CheckIsOpen();
            //System.IO.File.AppendAllText("C:/temps_.txt", obj.ToString());
